   ...
   <?php $idi = get_query_var( 'muokkaa', "0" );  ?>
    ...
    <div class="profile-info row">
      <div class="col-md-6">
        ...
        <dd class="col-sm-9"><?php echo get_the_title(get_field("breed", get_the_ID())); ?></dd>
        ...
        <dd class="col-sm-9"><?php echo get_field("height", get_the_ID()); ?>cm</dd>
        <dt class="col-sm-3">Syntymäaika</dt>
        <?php
          $originalDate = get_field("date_of_birth", get_the_ID());
          $syntymaaika = date("d.m.Y", strtotime($originalDate));
        ?>
        <dd class="col-sm-9"><?php echo $syntymaaika; ?></dd>
        ...
        <?php
          ...
          $pedigree = get_field("no_pedigree", get_the_ID());
        ?>
        <dd class="col-sm-9"><?php echo '<a href="' . get_site_url() . '/author/' . $user_info['nickname'] . '">' . $user_info['display_name'] . '</a>'; ?></dd>
        <dt class="col-sm-3">Kotitalli</dt>
        <dd class="col-sm-9"><?php if(get_field("stable_id", get_the_ID())) { echo get_the_title(get_field("stable_id", get_the_ID())); } else { echo '-'; } ?></dd>
        <dt class="col-sm-3">Kasvattaja</dt>
        <dd class="col-sm-9"><?php if($pedigree == '1') { echo "<em>evm</em>"; } else if(get_field("breeder_id", get_the_ID())) { echo get_field("breeder_id", get_the_ID()); } else { echo '-';} ?></dd>
        <dt class="col-sm-3">VH-tunnus</dt>
        <dd class="col-sm-9"><?php if(get_field("vh_id", get_the_ID())) { echo '<a href="http://www.virtuaalihevoset.net/?hevoset/hevosrekisteri/hevonen.html?vh=' . get_field("vh_id", get_the_ID()) . '">' . get_field("vh_id", get_the_ID()) . '</a>'; } else { echo '-';} ?></dd>
        <dt class="col-sm-3">Isä</dt>
        <?php $dad = get_field("sire_id", get_the_ID()); $dadp = get_field("sire_points", get_the_ID()); ?>
        <dd class="col-sm-9"><?php if($pedigree == '1') { echo "evm";}
          else if($dad) {
            echo '<a href="' . get_permalink($dad) . '" target="_blank">' . get_the_title($dad) . '</a>';
            if ($dadp == 0) { echo ' <small><em>ei otettu huomioon ensiarvostelussa</em></small>'; }
          }
          else { echo '<em>isää ei ole arvosteltu</em>';} ?></dd>
        <dt class="col-sm-3">Emä</dt>
        <?php $mom = get_field("dam_id", get_the_ID()); $momp = get_field("dam_points", get_the_ID()); ?>
        <dd class="col-sm-9"><?php if($pedigree == '1') { echo "evm";}
          else if($mom) {
          echo '<a href="' . get_permalink($mom) . '" target="_blank">' . get_the_title($mom) . '</a>';
          if ($momp == 0) { echo ' <small><em>ei otettu huomioon ensiarvostelussa</em></small>'; }
          }
        ...
        ?>
        <a class="btn btn-primary btn-sm" href="<?php echo get_field("url", get_the_ID()); ?>" target="_blank" role="button">Hevosen omat WWW-sivut</a>
        <?php } ?>
      </div>
      <div class="col-md-6">
        <div class="card mx-auto" style="width: 60%;">
          ...
          <div class="card-body text-center">
            <?php
            $grade = get_field("basic_evaluation_grade", get_the_ID());
            if ($grade == "1 Star Prospect") { echo '<img src="' . get_stylesheet_directory_uri() . '/img/1-star-prospect.png" alt="1 Star Prospect">'; }
            else if ($grade == "2 Star Prospect") { echo '<img src="' . get_stylesheet_directory_uri() . '/img/2-star-prospect.png" alt="2 Star Prospect">'; }
            else if ($grade == "3 Star Prospect") { echo '<img src="' . get_stylesheet_directory_uri() . '/img/3-star-prospect.png" alt="3 Star Prospect">'; }
            else if ($grade == "4 Star Prospect") { echo '<img src="' . get_stylesheet_directory_uri() . '/img/4-star-prospect.png" alt="4 Star Prospect">'; }
            else if ($grade == "5 Star Prospect") { echo '<img src="' . get_stylesheet_directory_uri() . '/img/5-star-prospect.png" alt="5 Star Prospect">'; }
            ?>
            <h5 class="card-title"><?php echo get_field("basic_evaluation_grade", get_the_ID()); ?></h5>
            <p class="card-text"><?php echo get_field("basic_evaluation_points", get_the_ID()); ?>/1900 pistettä</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="tab-content" id="pills-tabContent-sec">
          <div class="tab-pane fade show active" id="basic-evaluation" role="tabpanel" aria-labelledby="basic-evaluation-tab">
            ...
            <div class="profile-points row">
              ...
              <div class="col-md-6">
                <?php echo printPoints(get_field("points_behaviour", get_the_ID()), "Käytös tilaisuudessa"); ?>
              </div>
            </div>
          </div>
